                  ...
                  int enabled)
{
    ...
    D("%s: new_sensors=%08x ", __FUNCTION__, new_sensors);
    int result = write(file_handler, &new_sensors, sizeof(new_sensors));
	if(result<0)
	{
...
data__poll(struct sensors_poll_device_1 *dev, sensors_event_t* values)
{
    ...
    while (1) {
		...
		result = read(file_handler, output, sizeof(output));
		if(result<0)
		{
		...
		if(new_sensors)
		{
			...
			free(p_output);
			D("%s,  if(new_sensors) ", __FUNCTION__);
			return pick_sensor(data, values);
		}
        free(p_output);
		break;
            ...
            sensors_event_t* data, int count)
{
    ...
    for (i = 0; i < count; i++)  {
        ...
        D("%s, data__poll below ", __FUNCTION__);
        data++;
             ...
             struct hw_device_t*      *device)
{
    ...
    if (!strcmp(name, SENSORS_HARDWARE_POLL)) {
        ...
        *device = &dev->device.common;
        status  = 0;
        file_handler = open(DEVICENAME, O_RDWR);
		if(file_handler<0)
		{
			exit(1);
		}
